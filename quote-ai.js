...
(()=>{"use strict";
const C={phone:"2566385396",logo:"image-removebg-preview.png",key:"losBrosQuoteV3"};
const S={
...
if(k==="mowing"){
  let p=has(a.size,"small")?55:has(a.size,"medium")?75:has(a.size,"acre")?170:95;
  if(has(a.growth,"very"))p+=25;
  else if(has(a.growth,"little"))p+=0;
  if(has(a.slope,"steep"))p+=15;
  if(has(a.access,"no"))p+=10;
...
if(k==="weeds"){
  ...
  return Math.max(50,h*(has(a.hauling,"yes")?60:50));
}
if(k==="hedge"){
  let c=num(a.count,1),b=has(a.size,"small")?20:has(a.size,"medium")?35:has(a.size,"large")?50:35,p=c*b;
...
if(k==="mulch"){
  ...
  if(has(a.supply,"los bros")||has(a.delivery,"yes"))p+=100;
  return Math.max(75,p);
}
if(k==="bed"){
...
if(k==="cleanup"){
  ...
  if(has(a.access,"no"))h+=.5;
  return Math.max(60,h*(has(a.hauling,"yes")?60:50));
}
if(k==="plants"){
  let c=num(a.count,1),e=has(a.size,"small")?22:has(a.size,"medium")?40:has(a.size,"large")?75:40,p=Math.max(60,c*e);
...
if(k==="irrigation"){
  if(has(a.type,"new zone")||has(a.type,"new system"))return 750;
  let p=has(a.type,"diagnosis")?60:85;
...
if(k==="lighting"){
  let c=num(a.count,1);
  if(has(a.type,"repair"))return 100+Math.max(0,c-1)*20;
  let p=100+c*35;
  if(has(a.transformer,"no"))p+=125;
