(() => {
  ...
  if (window.__LOS_BROS_JUAN_AI_LOADED__) return;
  window.__LOS_BROS_JUAN_AI_LOADED__ = true;
  ...
  const CONFIG = {
    ...
    storageKey: "losBrosJuanQuoteV10",
    serviceAreas: [
      "Gadsden",
      "Rainbow City",
      "Glencoe",
  ...
  function loadState() {
    try {
      ...
      return saved && saved.version === 10 ? { ...defaultState(), ...saved } : defaultState();
    } catch {
      return defaultState();
    }
  }
  ...
  function getPageContext() {
    ...
    const path = `${window.location.pathname} ${document.title} ${heading}`.toLowerCase();
  ...
  function startConversation() {
    ...
    bot(`${context.message} I can answer questions, explain starting prices, or prepare a free quote request.`);
  ...
  function chooseService(key, fromOverlay = false) {
    ...
    state.phase = "service";
    state.estimate = null;
    state.estimateRange = null;
    state.estimateReasons = [];
    state.estimateConfidence = "medium";
  ...
  function showEstimatePreview() {
    ...
    const range = getSafePriceRange();

    bot(`Based on your answers, the fair starting range is $${range.low}–$${range.high}.`);

    const reasons = Array.isArray(state.estimateReasons)
      ? state.estimateReasons.slice(0, 4)
      : [];

    if (reasons.length) {
      bot(`What affected the range: ${reasons.join("; ")}.`);
    }

    const confidenceMessage = {
      high: "This range is fairly focused because you answered the main pricing questions.",
      medium: "This is a useful starting range, but pictures will help tighten it.",
      low: "This range is wider because some important details are still unknown."
    }[state.estimateConfidence] || "Pictures will help confirm the final price.";

    bot(confidenceMessage);
    showRecommendation();
    bot("Pictures and exact measurements can move the final price lower or higher. Would you like to finish the free quote request?");
  ...
  function showFinalCard() {
    ...
    // Prevent duplicate final cards and guarantee that preview/final/SMS match.
    messages.querySelectorAll(".lb-estimate-card").forEach(card => card.remove());
    const finalRange = getSafePriceRange();
    ...
    card.innerHTML = `
      <span class="lb-eyebrow">FAIR STARTING PRICE RANGE</span>
      <strong class="lb-estimate-price">$${finalRange.low}–$${finalRange.high}</strong>
      <small class="lb-range-label">Fair starting range • Final price confirmed after review</small>
      ...
      ${state.estimateReasons?.length ? `
        <div class="lb-price-reasons">
          <b>What affected this range</b>
          ${state.estimateReasons.slice(0, 5).map(reason => `<span>✓ ${escapeHtml(reason)}</span>`).join("")}
        </div>
      ` : ""}
  ...
  function smartReply(raw) {
    ...
    if (includesAny(q, ["free quote", "free estimate", "estimate free", "quote cost"])) {
      return "The quote request is free. The website provides a starting price range, and Los Bros Landscaping confirms the final price after reviewing the exact details and pictures.";
    }
    ...
    if (includesAny(q, ["exact price", "final price", "why estimate", "starting estimate", "price change"])) {
      return "The website shows a fair starting range, not a trick price and not a guaranteed final total. Photos can miss measurements, roots, slopes, access problems, buried debris, material quantities, and disposal volume. The final price is confirmed before work begins.";
    }
  ...
  function refreshSmartPricing() {
    if (!state.service || !SERVICES[state.service]) return null;

    const pricing = calculateSmartEstimate(state.service, state.answers || {});
    const minimum = getServiceMinimum(state.service);

    let mid = Number(pricing?.mid);
    let low = Number(pricing?.low);
    let high = Number(pricing?.high);

    if (!Number.isFinite(mid) || mid <= 0) mid = minimum;
    if (!Number.isFinite(low) || low <= 0) low = Math.max(minimum, mid - 20);
    if (!Number.isFinite(high) || high <= low) high = low + 20;

    // Never display fake ranges such as $50-$50.
    if (high - low < 10) high = low + 10;

    state.estimate = Math.round(mid / 5) * 5;
    state.estimateRange = {
      low: Math.round(low / 5) * 5,
      high: Math.round(high / 5) * 5
    };
    state.estimateReasons = Array.isArray(pricing?.reasons)
      ? pricing.reasons
      : [];
    state.estimateConfidence = pricing?.confidence || "medium";

    saveState();
    return {
      mid: state.estimate,
      low: state.estimateRange.low,
      high: state.estimateRange.high,
      reasons: state.estimateReasons,
      confidence: state.estimateConfidence
    };
  }

  function getSafePriceRange() {
    const existingLow = Number(state.estimateRange?.low);
    const existingHigh = Number(state.estimateRange?.high);

    if (
      Number.isFinite(existingLow) &&
      Number.isFinite(existingHigh) &&
      existingLow > 0 &&
      existingHigh > existingLow
    ) {
      return { low: existingLow, high: existingHigh };
    }

    const refreshed = refreshSmartPricing();
    if (refreshed) return { low: refreshed.low, high: refreshed.high };

    const fallback = Math.max(50, Number(state.estimate) || 50);
    return { low: fallback, high: fallback + 20 };
  }

  function calculateSmartEstimate(service, answers) {
    const mid = calculateEstimate(service, answers);
    const reasons = buildPriceReasons(service, answers);
    const unknowns = countUnknownPricingAnswers(service, answers);

    let spreadPercent = 0.16;
    let minimumSpread = 15;

    if (["cleanup", "weeds", "mulch", "bed"].includes(service)) {
      spreadPercent = 0.22;
      minimumSpread = 25;
    }

    if (["irrigation", "lighting"].includes(service)) {
      spreadPercent = 0.28;
      minimumSpread = 35;
    }

    spreadPercent += Math.min(0.14, unknowns * 0.025);

    const round5 = value => Math.max(0, Math.round(value / 5) * 5);
    let low = round5(mid * (1 - spreadPercent));
    let high = round5(mid * (1 + spreadPercent));

    low = Math.max(getServiceMinimum(service), low);
    high = Math.max(low + minimumSpread, high);

    const confidence =
      unknowns === 0 && !["irrigation", "lighting"].includes(service)
        ? "high"
        : unknowns <= 2
          ? "medium"
          : "low";

    return { low, high, mid, reasons, confidence };
  }

  function getServiceMinimum(service) {
    return {
      mowing: 50,
      weeds: 50,
      hedge: 40,
  ...
  function countUnknownPricingAnswers(service, answers) {
    const questions = SERVICES[service]?.questions || [];
    return questions.reduce((count, question) => {
      const value = answers?.[question[0]];
      return count + (!value || isUnknown(value) || value === "Skipped" ? 1 : 0);
    }, 0);
  }

  function buildPriceReasons(service, a) {
    const reasons = [];
    const add = (condition, text) => {
      if (condition && !reasons.includes(text)) reasons.push(text);
    };

    switch (service) {
      case "mowing":
        add(has(a.size, "small"), "small-yard base price");
        add(has(a.size, "medium"), "medium yard size");
        add(has(a.size, "large"), "large yard size");
        add(has(a.size, "acre"), "one acre or more");
        add(has(a.growth, "little"), "slightly overgrown grass");
        add(has(a.growth, "very"), "heavy overgrowth");
        add(has(a.slope, "some"), "sloped areas");
        add(has(a.slope, "steep"), "steep terrain");
        add(no(a.access), "limited equipment access");
        add(has(a.obstacles, "some"), "several obstacles");
        add(has(a.obstacles, "a lot"), "many obstacles or tight areas");
        add(has(a.extras, "bag"), "grass bagging and disposal");
        add(has(a.extras, "leaf"), "light leaf cleanup");
        add(has(a.frequency, "weekly"), "recurring weekly service");
        break;

      case "weeds":
        add(has(a.density, "moderate"), "moderate weed coverage");
        add(has(a.density, "thick"), "thick weed growth");
        add(has(a.density, "completely"), "area fully covered by weeds");
        add(includesAny(a.kind, ["vine", "deep", "mixed"]), "vines or difficult roots");
        add(yes(a.digging), "root digging");
        add(yes(a.hauling), "debris hauling");
        add(no(a.access), "difficult carrying access");
        break;

      case "hedge":
        add(has(a.size, "small"), "small bushes");
        add(has(a.size, "medium"), "medium bushes");
        add(has(a.size, "large"), "large bushes");
        add(has(a.cut, "moderate"), "moderate trimming");
        add(has(a.cut, "heavy"), "heavy cutback");
        add(yes(a.high), "sections above safe ground reach");
        add(yes(a.hauling), "clipping cleanup and hauling");
        add(yes(a.obstacle), "fence, wall, or other obstacles");
        break;

      case "control":
        add(has(a.density, "quarter"), "about one-quarter weed coverage");
        add(has(a.density, "half"), "about half weed coverage");
        add(has(a.density, "most"), "heavy weed coverage");
        add(includesAny(a.where, ["several", "lawn"]), "larger or multiple treatment areas");
        add(yes(a.plants), "care needed around nearby plants");
        add(includesAny(a.weed, ["dallis", "nutsedge", "crabgrass", "vine"]), "more difficult weed type");
        break;

      case "mulch":
        add(no(a.ready) || has(a.ready, "partly"), "bed preparation");
        add(yes(a.old), "old mulch removal");
        add(yes(a.weeds), "weed removal");
        add(yes(a.edge), "bed-edge cleanup");
        add(yes(a.fabric), "landscape fabric installation");
        add(no(a.access), "longer material carrying distance");
        add(has(a.supply, "los bros"), "material pickup or supply");
        add(yes(a.delivery), "delivery");
        break;

      case "bed":
        add(has(a.new, "completely"), "building a completely new bed");
        add(yes(a.grass), "grass or sod removal");
        add(yes(a.weeds), "weed and root removal");
        add(has(a.edge, "installed"), "installed edging");
        add(yes(a.plants), "plant installation");
        add(yes(a.soil), "new soil");
        add(has(a.supply, "los bros"), "material supply");
        add(no(a.access), "limited material access");
        break;

      case "cleanup":
        add(has(a.severity, "moderate"), "moderate cleanup");
        add(has(a.severity, "heavy"), "heavy cleanup");
        add(has(a.severity, "very heavy"), "very heavy cleanup");
        add(has(a.area, "large") || has(a.area, "several"), "large or multiple work areas");
        add(yes(a.vines), "vines or briars");
        add(yes(a.digging), "digging and root removal");
        add(yes(a.heavy), "heavy or bulky debris");
        add(yes(a.hauling), "hauling and disposal");
        add(no(a.access), "limited trailer or vehicle access");
        break;

      case "plants":
        add(has(a.size, "large"), "large plants");
        add(yes(a.remove), "old plant or root removal");
        add(has(a.soil, "hard") || has(a.soil, "rocky"), "hard or rocky soil");
        add(yes(a.products), "soil products or mulch");
        add(has(a.supply, "los bros"), "plant supply");
        add(yes(a.delivery), "plant delivery");
        add(yes(a.prep), "bed preparation");
        break;

      case "irrigation":
        add(has(a.type, "diagnosis"), "system diagnosis");
        add(has(a.type, "repair"), "repair work");
        add(has(a.type, "new zone"), "new irrigation zone");
        add(has(a.type, "new installation"), "new system installation");
        add(yes(a.leak), "visible leak or pooling");
        add(yes(a.underground), "possible underground pipe or wiring");
        add(no(a.valve), "valve location is unknown");
        break;

      case "lighting":
        add(has(a.type, "repair"), "lighting repair");
        add(has(a.type, "add"), "adding to an existing system");
        add(has(a.type, "new"), "new lighting installation");
        add(no(a.transformer), "new transformer may be needed");
        add(no(a.power), "outdoor power access is limited");
        add(has(a.route, "hard") || has(a.route, "several"), "difficult wiring route");
        add(yes(a.crossing), "driveway or walkway crossing");
        add(has(a.supply, "los bros"), "fixture supply");
        break;
    }

    if (!reasons.length) reasons.push("standard labor for the selected service");
    return reasons;
  }
  ...
  function sendQuoteText() {
    ...
    const lines = [
      ...
      `STARTING RANGE SHOWN: $${finalRange.low}-$${finalRange.high}`,
      ""
    ];
  ...
  function bot(text, options = {}) {
    ...
    typingQueue.push({
      text: cleanText,
      delay: Number(options.delay) || Math.min(520, Math.max(180, cleanText.length * 3.2))
    });
  ...
  function processTypingQueue() {
    ...
    typing.setAttribute("aria-label", "Juan is typing");
    ...
    typingTimer = setTimeout(() => {
      typing.remove();
      addBubble("bot", next.text, true);
      scrollBottom();
      typingActive = false;
      setTimeout(processTypingQueue, 70);
    }, next.delay);
  }

  function removeTypingIndicators() {
    messages.querySelectorAll(".lb-typing-row").forEach(el => el.remove());
  }
