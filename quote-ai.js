(() => {
  ...
  function loadState() {
    try {
      ...
      return saved && saved.version === 9 ? { ...defaultState(), ...saved } : defaultState();
    } catch {
      return defaultState();
    }
  }
  ...
  function showEstimatePreview() {
    ...
    const range = state.estimateRange || {
      low: Math.max(0, Number(state.estimate || 0) - 20),
      high: Number(state.estimate || 0) + 20
    };
  ...
  function showFinalCard() {
    ...
    card.innerHTML = `
      ...
      <strong class="lb-estimate-price">$${state.estimateRange?.low ?? state.estimate}–$${state.estimateRange?.high ?? state.estimate}</strong>
  ...
  function sendQuoteText() {
    ...
    const lines = [
      ...
      `STARTING RANGE SHOWN: $${state.estimateRange?.low ?? state.estimate}-$${state.estimateRange?.high ?? state.estimate}`,
      ""
    ];
